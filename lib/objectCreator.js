  ...
  for (let i = 0; i < objectBlueprint.length; i++) {
    if (objectBlueprint[i].parents.length > 0) {
      ...
      if (!isKeyAlreadyExists(CURRENT_OBJECT, objectBlueprint[i].keyAndValue.key)) {
        ...
        const formattedValue = eolFilteredValue.trim();
        // eval(`CURRENT_OBJECT.${objectBlueprint[i].keyAndValue.key} = formattedValue`);
        CURRENT_OBJECT[objectBlueprint[i].keyAndValue.key] = formattedValue;
      }
    } else {
