function checkKeyDuplicate (blueprintForCreatingJSON) {
  ...
  for (let index1 = 0; index1 < blueprintForCreatingJSON.length; index1++) {
    ...
    for (let index2 = 0; index2 < blueprintForCreatingJSON.length; index2++) {
      ...
      if (IS_MAIN_KEY_DUPLICATED) {
        console.log('key', blueprintForCreatingJSON[index2].keyAndValue.key);
        console.log('value', blueprintForCreatingJSON[index2].keyAndValue.value);
        DUPLICATED_KEY = `${blueprintForCreatingJSON[index2].keyAndValue.key}:${blueprintForCreatingJSON[index2].keyAndValue.value}`;
        console.log()
        break;
      }
    }
  ...
  console.log('RESULT:', DUPLICATED_KEY);
  return {
