...
module.exports = {
  async userInfoJsonSelecter (jsonFiles = []) {
    let questions = [
      {
        type: 'list',
        name: 'selectedJson',
        message: 'エクセルに変換するJSONファイルを選択してください',
        choices: jsonFiles
      }
    ];
    return INQUIRER.prompt(questions);
  },
  async userInfoExcelFileName () {
    let questions = [
      {
        type: 'input',
        name: 'excelFileName',
        message: 'エクセルのファイル名を入力してください',
        validate: function (input) {
          if (input.length === 0) {
            return 'エクセルファイル名は必須項目です。入力してください';
          } else {
            return true;
          }
        },
  ...
  async userInfoExcelFileSheetName () {
    let questions = [
      {
        type: 'input',
        name: 'sheetName',
